...
private:
  ...
  std::map<std::string, Mutex*> mutexMap;

public:
  MutexManager(); // Обычный конструктор
  //MutexManager(const MutexManager& other); // Конструктор копирования
  ~MutexManager();
  ...
  bool addMutex(const std::string& mutexName, std::string &errorMsg);
  Mutex *getMutex(const std::string& mutexName);
  void clear();
  ...
  bool destroyMutex(const std::string &mutexName, std::string &errorMsg);
};
